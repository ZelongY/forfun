...
'''
def update_service_metadata(wms):   
    attributes = {key:value for key, value in wms.identification.__dict__.items() if not key.startswith('_') and not callable(key)}
    
    for key, value in attributes.items():
        print key, value
        
    contact_attributes = {key:value for key, value in wms.identification.__dict__.items() if not key.startswith('_') and not callable(key)}
